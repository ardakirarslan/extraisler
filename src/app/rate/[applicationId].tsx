  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (appError || !application) {
        if (!cancelled) {
          setBlockedReason('Başvuru bulunamadı.');
          setLoading(false);
        }
        return;
      }
      ...
      if (application.status !== 'completed') {
        if (!cancelled) {
          setBlockedReason('Bu başvuru henüz tamamlanmadı.');
          setLoading(false);
        }
        return;
      }
      ...
      if (!jobPost) {
        if (!cancelled) {
          setBlockedReason('İlan bulunamadı.');
          setLoading(false);
        }
        return;
      }
      ...
      if (userId === application.worker_id) {
        target = jobPost.employer_id;
        role = 'worker';
      } else if (userId === jobPost.employer_id) {
        target = application.worker_id;
        role = 'employer';
      }

      if (!target || !role) {
        if (!cancelled) {
          setBlockedReason('Bu başvuruyu değerlendirme yetkin yok.');
          setLoading(false);
        }
        return;
      }
      ...
      if (existing) {
        if (!cancelled) {
          setBlockedReason('Bu başvuruyu zaten değerlendirdin.');
          setLoading(false);
        }
        return;
      }
